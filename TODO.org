...
* TODO Should not be able to serialize VMs 
...
* TODO clean up the exported symbols
  (the user should only need to use functions exported from the
  'shovel' package)
** TODO the run-vm should allow a parameter 'vm'
** TODO there should be a way to wake up a vm (clear 'should-take-a-nap')
* TODO publish the functions to extract the context from a VM
