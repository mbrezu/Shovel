...
** TODO reserve keyword 'block'
   used as in 'block name { ... block ... }'; starts a named block and
   uses instruction 'BLOCK';
** TODO reserve keyword 'block_return'
   used as in 'block_return name, return-value'; pushes *return-value*
   on the stack and 'returns' to block *name* using instruction
   'BLOCK_RETURN';
** TODO add VM instruction 'BLOCK name, address'
   which pushes a 'block' structure made of *name*, *address* and the
   current environment on the stack; note: a 'POP_BLOCK' must be
   executed right after *address*;
** TODO add VM instruction 'BLOCK_RETURN name'
   ...
   stack, the 'next top' is the value pushed by the corresponding
   BLOCK instruction; everything found in between these on the stack
   is deleted;
** TODO add VM instruction 'POP_BLOCK'
   it pops the 'next top' of the stack, in case it is a block, error
   otherwise; must be paired with a 'BLOCK' instruction, which it
   cancels.
* TODO documentation
