...
* TODO VM execution quotas for 'run-vm'
...
** TODO test shovel-vm:*cells-increment-herald* calls
** DONE the number of ticks executed so far must be made part of the
   serialized VM state;
** DONE execution quotas basic
** TODO limit integers to a known amount (60 bits?)
   unbounded bignums make it very hard to limit memory and CPU usage
** TODO implement primitive quotas by function calls
   - incrementTicks
   - incrementCells
   - askPermissionForCells
* TODO move SHRIPT-FILE and SHOVEL-ERROR and errors to a
  SHOVEL-PUBLIC-TYPES package and file;
* TODO the 'pop' required primitive should set the array item to null before
  decrementing the fill pointer (so the CL GC can reclaim the object
  pointed to by that array element)
* TODO use #-/#+ in the asd file to eliminate real-tests.lisp
* TODO provide a public getter for total-executed-ticks
* TODO try to optimize step-vm by building a jump table with functions
* TODO remove references to Shript and rename it ShovelScript
